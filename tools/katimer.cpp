...
{
	const unsigned Divs[] PROGMEM = { 1, 8, 64, 256, 1024 };
	const unsigned Caps[] PROGMEM = { 255, 65535 };

	register unsigned long	Cap;
	register unsigned short	i;

	for (i = 0; i < 5; i++) if ((Cap = (F_CPU / __LPM_word(&Divs[i])) / FreqA - 1) < __LPM_word(&Caps[ID])) break;

	Count	=	(Cap > __LPM_word(&Caps[ID])) ? 0 : Cap;
	Prev		=	(FreqB && Count) ? (F_CPU / __LPM_word(&Divs[i])) / FreqB - 1 : 0;
	Scale	=	i + 1;
}
...
void KATimer::SetTime(unsigned long TimeA, unsigned long TimeB)
{
	...
	for (i = 0; i < 5; i++) if ((Cap = (F_CPU / 1000000) * (TimeA / __LPM_word(&Divs[i])) - 1) < __LPM_word(&Caps[ID])) break;

	Count	=	(Cap > __LPM_word(&Caps[ID])) ? 0 : Cap;
	Prev		=	(TimeB && Count) ? (F_CPU / 1000000) * (TimeB / __LPM_word(&Divs[i])) - 1 : 0;
	Scale	=	i + 1;
}
